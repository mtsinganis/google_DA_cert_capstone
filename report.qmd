...
Next, I rename each of the data frames according to their corresponding date in the format 'YYYY_MM': 

```{r rename-list-elements}
...
Let's take a first look at the first element of the list (first data frame):
...
I order to start exploring the data and getting insights into their structure, I want to combine all the data frames into one. Before I do that however, I need to confirm whether they all have same number of columns and identical column names.
...
trips %>%
    group_by(yyyy_mm, member_casual) %>% 
    summarise(n = n()) %>% 
    ggplot(aes(x = yyyy_mm, y = n,
               group = member_casual,
               color = member_casual)) +
    geom_line() +
...
```
The number of bike trips shows clear seasonality, with a peak in July and a trough in January, suggesting bike usage is highly dependent on general weather conditions. This seasonality is observed for both casual riders and members.
