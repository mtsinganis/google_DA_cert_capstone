...
```
They also have identical column names. This allows me to combine them by row in one large data frame (essentially stacking them).
...
#ym(trips$yyyy_mm[1]) # convert to date
...
I now check how many bike trips were registered by month.

```{r}
trips %>%
    group_by(yyyy_mm, member_casual) %>% 
    summarise(n = n()) %>% 
    ggplot(aes(x = yyyy_mm, y = n)) + geom_line(aes(group = member_casual)) +
    theme(axis.text.x = element_text(angle = 90))

```
The number of bike trips shows clear seasonality, suggesting bike usage is
